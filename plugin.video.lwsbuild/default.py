...
base='https://archive.org/download/LwSRepo/'
ADDON=xbmcaddon.Addon(id='plugin.video.lwsbuild')
...
def CATEGORIES():
    link = OPEN_URL('http://archive.org/download/LwSRepo/wizard_test.txt').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("LwS Wizard","Downloading ",'', 'Please Wait')
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("DOWNLOAD COMPLETE YAY", 'Unfortunately the only way to get the new changes to stick is', 'to force close kodi. Click ok to force Kodi to close,', 'DO NOT use the quit/exit options in Kodi., If the Force close does not close for some reason please Restart Device or kill task manaully')
    killxbmc()
