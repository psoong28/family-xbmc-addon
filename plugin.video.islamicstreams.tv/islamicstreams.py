...
# The purpose of this plugin is to provide a live streaming of Islamic Streams TV.
...
addon = xbmcaddon.Addon(id='plugin.video.islamicstreams.tv')
...
data = 'https://raw.githubusercontent.com/indiecrew/testing/master/ISTV.xml'

def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.islamicstreams.tv-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Islamic Streams TV !, We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Islamic Streams TV !, We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
