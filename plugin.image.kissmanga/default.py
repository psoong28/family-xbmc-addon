...
def listLinks(section, url, showtitle='', showyear=''): ### Menu for Listing Hosters (Host Sites of the actual Videos)
	...
	#s='lstImages\.push\("(http\D*://.+?\..+?/.+?/[A-Za-z\-_]*(\d*)[A-Za-z\-_]*\.\D+[^"]*)"\);'
	s='lstImages\.push\("(http\D*://.+?/[A-Za-z\-_]*(\d+)[A-Za-z\-_]*\.(?:png|jpg|jpeg|PNG|JPG|JPEG|gif|GIF)[^"]*)"\);'
	...
	if (len(matches) > 0):
		...
		for ThisMatch in matches:
			#mUrl,mNo,imgmax=ThisMatch
			mUrl,mNo=ThisMatch; imgmax=''
