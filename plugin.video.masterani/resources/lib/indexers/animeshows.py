...
class Indexer:
    def __init__(self):
        ...
        self.posterurl = "http://cdn.masterani.me/poster/1/%s"
    ...
    def get_by_select(self, stype=[], sort=0, status=0, genre=[], offset=0):
        self.list = []

        if offset is None: offset = 0
        offset = int(offset)

        glist = masterani.get_by_select(genre=genre, sort=sort, status=status, stype=stype)
        self.totalitems = len(glist)

        if offset > self.totalitems:
            glist = glist[offset: self.totalitems]
        else:
            glist = glist[offset: offset + self.itemsperpage]

        if len(glist) is 0:
            return

        for i in glist:
            self.list.append({'anime_id': i[0], 'status': i[1]})

        self.worker()
        self.add_directory(self.list, sgenre=genre, sort=sort, status=status, stype=stype, offset=offset)
    ...
    def add_directory(self, items, stype=[], sort=0, status=0, sgenre=[], offset=0):
